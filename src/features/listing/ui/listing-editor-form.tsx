...
const editorCopy = {
  create: {
    description: "Заполните информацию о товаре или услуге",
    title: "Создание объявления",
  },
  edit: {
    description: "Обновите данные объявления, фотографии, адрес и характеристики в одной форме.",
    title: "Редактирование объявления",
  },
} satisfies Record<ListingEditorFormProps["mode"], {
  description: string;
  title: string;
}>;

export function ListingEditorForm({ mode, state }: ListingEditorFormProps) {
  const {
    form: {
      formState: { errors },
      register,
    ...
    <div className="grid gap-8">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.7fr)_minmax(360px,1fr)]">
        <section className="rounded-[32px] border border-[var(--border-soft)] bg-[color-mix(in_srgb,var(--surface)_94%,transparent)] p-6 shadow-[var(--shadow-card)] sm:p-7">
          <div className="grid gap-8">
            <ListingCategorySection segment="type" state={state} variant={mode} />
            ...
            <ListingPricingSection state={state} variant={mode} />
            <ListingCategorySection state={state} variant={mode} />
            <ListingAddressSection state={state} />
          </div>
        </section>

        <div className="grid gap-5">
          <ListingAttributesSection state={state} variant={mode} />
        </div>
      </div>

      <ListingMediaSection state={state} variant={mode} />
      <ListingSubmitBar state={state} variant={mode} />
    </div>
  );
  ...
  if (mode === "edit") {
    return (
      <section className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-heading text-2xl font-black text-[var(--brand-deep)]">
            {title}
          </h1>

...
src/widgets             крупные композиционные блоки (header, layout, account shell)
...
Зависимости направлены от верхних слоев к нижним. В частности, production-код
`shared` не может импортировать `app`, `widgets`, `screens`, `features` или
`entities`. Ограничение применяется автоматически ESLint-правилом
`no-restricted-imports` ко всем TypeScript-файлам слоя. Межслойные API contract
tests находятся в `tests/contracts`, поскольку проверяют интеграцию нескольких
слоев и не принадлежат одному FSD slice.
...
Глобальный provider не должен проверять `/auth/me` на каждом анонимном public
просмотре. Lazy bootstrap использует локальный булевый
`snabix:auth-session-hint:v1`: он не содержит user ID, email, cookie, access
token или другие персональные данные и не является доказательством
авторизации. Источником истины остается backend session:

- `/account` и все вложенные маршруты всегда могут инициировать `/auth/me`;
- public route проверяет `/auth/me` только при наличии ранее подтвержденного hint;
- успешный `getMe`/sign-in сохраняет hint, а logout, `401`, `419` и завершение
  текущей сессии удаляют его;
- ошибка или недоступность localStorage не блокирует auth flow.

Header получает уведомления только для подтвержденного пользователя и только
когда вкладка видима и browser online. Scheduler не допускает параллельных
одинаковых запросов, обновляет feed при возвращении focus/online и открытии
меню, а после ошибок увеличивает интервал с 30 секунд максимум до 5 минут.
Успешный ответ сбрасывает backoff. Все notification mutations проходят через
единый error boundary с пользовательским toast и не оставляют rejected Promise
без обработчика.
